...
void main() {
  ...
  // Reconstruct a crude surface slope from depth to find flat ground.
  float dC = linearDepth(vUv);
  float dU = linearDepth(vUv + vec2(0.0, uTexel.y * 2.0));
  float dD = linearDepth(vUv - vec2(0.0, uTexel.y * 2.0));
  float slope = abs(dU - dD);
  float flat_ = 1.0 - smoothstep(0.02, 0.25, slope / max(dC, 1.0));
  float ground = smoothstep(0.62, 0.30, vUv.y) * flat_;
  float amount = ground * uStrength * (0.30 + uWet * 0.70);
  if (amount < 0.01 || dC > uFar * 0.4) { gl_FragColor = base; return; }

  // Mirror upward from the horizon and blur slightly with distance.
  float horizon = 0.62;
  vec2 ruv = vec2(vUv.x, horizon + (horizon - vUv.y) * 0.92);
  ruv.x += sin(vUv.y * 90.0) * 0.0016 * uWet;
  vec3 refl = vec3(0.0);
  refl += texture2D(tDiffuse, ruv).rgb * 0.5;
  refl += texture2D(tDiffuse, ruv + vec2(uTexel.x * 2.0, 0.0)).rgb * 0.25;
  refl += texture2D(tDiffuse, ruv - vec2(uTexel.x * 2.0, 0.0)).rgb * 0.25;
  float fade = smoothstep(0.0, 0.25, ruv.y) * step(ruv.y, 1.0);
  gl_FragColor = vec4(mix(base.rgb, refl, clamp(amount * fade, 0.0, 0.55)), base.a);
}`;
